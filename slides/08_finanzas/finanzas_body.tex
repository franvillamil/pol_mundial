...
\begin{itemize}
  ...
  \item Interdependencia armada: las finanzas como instrumento de poder
\end{itemize}
\end{frame}
% ----------------------------------------------------
\note{}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/markets}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
Flujos internacionales de capital: \textbf{inversión}\\
\end{transitionframe}
% ----------------------------------------------------
\note{}
...
\begin{tabular}{>{\bfseries}p{2.8cm} p{3.5cm} p{3cm}}
 & \textbf{\textcolor{accent}{Inversión en cartera}} & \textbf{\textcolor{accent2}{Inversión extranjera directa (FDI)}} \\[6pt]
\hline
...
\begin{itemize}
  ...
  \begin{itemize}
    \item La mayor parte del FDI va de países ricos a países ricos
...
% ----------------------------------------------------
\begin{frame}
\frametitle{FDI y las multinacionales}
\centering

\begin{itemize}
  \item FDI: la realizan principalmente \textbf{empresas multinacionales}
  \vspace{8pt}
  \item Motivaciones de las multinacionales:
  \begin{itemize}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/iphone_india}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
% ----------------------------------------------------
\begin{transitionframe}
Flujos internacionales de capital: \textbf{deuda}
\end{transitionframe}
% ----------------------------------------------------
\note{}
...
\begin{itemize}
  \item Cuando la deuda se vuelve insostenible, el ajuste es inevitable y lo suelen demandar los prestamistas (FMI)
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/greece_austerity}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
% ----------------------------------------------------
\begin{transitionframe}
El \textbf{dinero}\\{\small Tipos de cambio y sistemas monetarios}
\end{transitionframe}
% ----------------------------------------------------
\note{}
...
\begin{tikzpicture}[>=stealth, scale=0.9]
  % Triangle vertices
  \coordinate (A) at (-1, 3.5);   % top: tipo fijo
  \coordinate (B) at (-4, 0);  % bottom-left: libre capital
  \coordinate (C) at (2, 0);   % bottom-right: pol. monetaria
  ...
  \node[right, font=\small\bfseries, align=left] at (C) {Política monetaria\\independiente};

  % Country examples on each side (which two you pick)
  \node[left, font=\scriptsize, text=accent, align=center] at (-2.5, 1.75)
    {Eurozona\\(fijo + libre capital)};
  \node[below, font=\scriptsize, text=asher, align=center] at (-1, -0.15)
    {EE.UU. / UK\\(libre capital + pol. monetaria)};
  \node[right, font=\scriptsize, text=accent2, align=center] at (0.75, 1.75)
    {China 2000s\\(fijo + pol. monetaria)};
\end{tikzpicture}

\vspace{4pt}
\alert{Solo puedes elegir dos de los tres:} cada par excluye el tercero

\end{frame}
% ----------------------------------------------------
\note{El trilema (o trinidad imposible) de la política monetaria es uno de los conceptos más importantes de la economía internacional. Es el primo hermano del trilema de Rodrik que vimos en la sesión anterior: allí no se podía tener hiperglobalización, soberanía y democracia al mismo tiempo; aquí no se puede tener tipo de cambio fijo, libre movimiento de capitales, y política monetaria autónoma. La misma lógica de restricciones imposibles. Si tienes tipo fijo y libre movimiento de capitales, tu política monetaria debe estar supeditada a mantener la paridad (como pasó en Argentina). Si tienes libre movimiento de capitales y política monetaria autónoma, el tipo de cambio debe flotar. Si quieres tipo fijo y política monetaria autónoma, tienes que controlar los capitales (como hacía China hasta los años 2000). Los países de la eurozona han elegido tipo fijo (moneda única) y libre movimiento de capitales, renunciando a la política monetaria nacional.}
...
\begin{tikzpicture}[>=stealth, thick, node distance=1.6cm]
  ...
  \node[font=\scriptsize, align=center] at (9.2, 2) {\textit{Profecía}\\\textit{autocumplida}};
  \draw[->, dashed] (8.5, 2.1) -- (6.8, 2.4);
\end{tikzpicture}
...
% ====================================================
\section{Weaponized interdependence}
% ====================================================

% ----------------------------------------------------
\begin{transitionframe}
Globalización, interdependencia, y coacción\\
{\small Abraham Newman \& Henry Farrell}
\end{transitionframe}
% ----------------------------------------------------
\note{}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{\textit{Weaponized interdependence} {\footnotesize (interdependencia instrumentalizada)}}
\centering

% ----------------------------------------------------
\begin{minipage}{0.6\textwidth}\centering
  \begin{itemize}
  \item Farrell y Newman (2019)
  \item Las redes económicas globales tienen \textbf{estructura de hub}
  \begin{itemize}
    \item No son planas ni simétricas: hay \alert{nodos centrales} que controlan los flujos
  \end{itemize}
  \vspace{8pt}
  \item<2-> Los estados que controlan esos nodos pueden \textbf{instrumentalizarlos}:
  \begin{itemize}
    \item \textbf{Vigilar} los flujos (inteligencia financiera)
    \item \textbf{Cortar acceso} a la red (sanciones)
  \end{itemize}
\end{itemize}
...
\begin{minipage}{0.39\textwidth}\centering
    \includegraphics[width = \textwidth]{../img/underground_empire}
\end{minipage}
% ----------------------------------------------------
...
\end{frame}
% ----------------------------------------------------
\note{Henry Farrell y Abraham Newman publicaron ``Weaponized Interdependence'' en \textit{International Security} en 2019, y luego expandieron la tesis en \textit{Underground Empire} (2023). Su argumento central desafía la visión liberal clásica de que la interdependencia económica fomenta la paz y la cooperación. En lugar de eso, muestran que las redes económicas globales tienen una topología de hub-and-spoke: no son planas, sino que los flujos pasan por nodos centrales. Los estados que controlan esos nodos (sobre todo EE.UU.) pueden explotarlos de dos formas: (1) el efecto panóptico, que permite vigilar las transacciones (así descubrió EE.UU. la red de financiación de Al Qaeda tras el 11-S, accediendo a datos de SWIFT); y (2) el efecto de estrangulamiento, que permite cortar el acceso de un actor a la red global (lo que las sanciones hacen con Irán, Rusia o Corea del Norte). El dólar es el nodo más poderoso: al ser la moneda de reserva global, casi todas las transacciones internacionales pasan por bancos americanos en algún punto, lo que da a Washington jurisdicción extraterritorial de facto.}

% ----------------------------------------------------
\begin{frame}
\frametitle{El caso de EEUU}
\centering

  \begin{itemize}[<+->]
  \item EEUU ha estado usando cada vez su contorl de \textbf{nodos clave}:
  \begin{itemize}
    \item \textbf{Dólar:} el 88\% de las transacciones de divisas usan dólares
    \item \textbf{SWIFT:} sistema de mensajería interbancaria global
    \item \textbf{Semiconductores:} cadena de suministro concentrada (TSMC, ASML)
    \item \textbf{Internet:} cables submarinos, servidores DNS, plataformas
  \end{itemize}
  \item Gran parte de este control no se consiguió conscientemente, sino de manera indirecta (el mejor ejemplo, internet)
  \item Los gobiernos americanos han aprendido poco a poco el control que tienen
\end{itemize}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Sanciones financieras: la interdependencia como arma}
\centering

\begin{itemize}
  \item \textbf{Irán (2012--):} desconectado de SWIFT, sin acceso al sistema de pagos en dólares
  \begin{itemize}
    \item Exportaciones de petróleo cayeron un 60\%
  \end{itemize}
  \vspace{6pt}
  \item<2-> \textbf{Rusia (2022):} tras la invasión de Ucrania
  \begin{itemize}
    \item Exclusión parcial de SWIFT, congelación de 300.000 M\$ de reservas del banco central
    \item Las sanciones más amplias jamás impuestas a una gran economía
  \end{itemize}
  \vspace{6pt}
  \item<3-> \textbf{Contraofensiva:} los países sancionados buscan alternativas
  \begin{itemize}
    \item China: sistema CIPS (alternativa a SWIFT), yuan digital
    \item Rusia-China: comercio bilateral en yuanes
    \item Debate sobre \textit{dedolarización}: ¿se fragmentará el sistema?
  \end{itemize}
  \vspace{6pt}
  \item<4-> \alert{Paradoja:} cuanto más se usan las redes como arma, más incentivos hay para crear redes alternativas
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Los ejemplos concretos muestran cómo funciona la interdependencia armada. Irán fue el primer caso a gran escala: en 2012, EE.UU. y la UE desconectaron a los bancos iraníes de SWIFT, haciendo extremadamente difícil para Irán vender petróleo o recibir pagos internacionales. La presión fue tan efectiva que contribuyó a llevar a Irán a la mesa de negociación del acuerdo nuclear de 2015 (JCPOA). Rusia en 2022 es el caso más extremo: la congelación de 300.000 millones de dólares en reservas del banco central ruso fue sin precedentes. Ningún país del G20 había sufrido algo así. Pero hay una paradoja: el uso agresivo de estos instrumentos incentiva a los países afectados (y a otros que temen ser los siguientes) a construir redes alternativas. China ha desarrollado el CIPS como alternativa a SWIFT, está promoviendo el uso del yuan en comercio bilateral, y experimenta con un yuan digital. Si la fragmentación avanza, EE.UU. podría perder precisamente la ventaja que hace poderosas sus sanciones. Farrell y Newman llaman a esto el ``dilema del imperio subterráneo''.}

% ----------------------------------------------------
\imageframe{../img/petroyuan}
\note{}
% ----------------------------------------------------
...
\begin{itemize}\small
  ...
  \item La FDI la realizan \textbf{multinacionales}; la reshoring y friendshoring reflejan tensiones geopolíticas
  ...
  \item La \textbf{interdependencia armada}: las redes financieras globales pueden usarse como instrumento de coerción
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{Resumen integrador de los conceptos de la sesión. El hilo conductor es que cada dimensión de las finanzas internacionales --- inversión, deuda, tipos de cambio, crisis --- tiene una estructura distributiva: hay ganadores y perdedores, y la política determina quién carga con los costes. La inversión favorece a los países con bajo riesgo político. La deuda soberana enfrenta a prestamistas con deudores y ciudadanos. Los tipos de cambio dividen a exportadores de consumidores. Las crisis financieras se contagian y a menudo el coste lo pagan los más vulnerables. Además, la interdependencia económica no es solo una fuente de prosperidad compartida: como muestran Farrell y Newman, las redes financieras globales pueden convertirse en instrumentos de coerción para los estados que controlan sus nodos centrales.}
