...
\begin{itemize}
  ...
  \item El puzzle: \textquestiondown por qu\'{e} unos pa\'{i}ses son ricos y otros pobres?
  ...
  \item El modelo chino y la trampa de la renta media
  \item \textquestiondown Ayuda o comercio?
\end{itemize}
\end{frame}
% ----------------------------------------------------
\note{Seis bloques en la sesion. Empezar con el puzzle (por que la diferencia entre paises ricos y pobres) para motivar la pregunta. Luego factores domesticos (geografia, intereses, recursos) e instituciones como explicaciones centrales. Despues el entorno internacional (colonialismo, terminos de intercambio, instituciones sesgadas). Las estrategias de desarrollo (ISI, EOI, Consenso de Washington) muestran las respuestas politicas a lo largo del tiempo. Cerrar con el debate ayuda vs. comercio, que conecta todo. Tiempo estimado: 10 min puzzle, 15 min factores domesticos, 10 min instituciones, 15 min entorno internacional, 15 min estrategias, 10 min debate final. Conectar con la sesion anterior sobre finanzas: hoy vemos el otro lado de la moneda --- por que el capital y las oportunidades no se distribuyen de forma igualitaria.}

% ====================================================
\section{El puzzle del desarrollo}
% ====================================================

% ----------------------------------------------------
\begin{transitionframe}
El puzzle del desarrollo
\end{transitionframe}
% ----------------------------------------------------
\note{}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, width=\paperwidth]{../img/global_gdp}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textit{Dutch disease} (síndrome holandés): recursos aprecian la moneda, el resto pierde competitividad
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/southern_africa_gdp}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, width=\paperwidth]{../img/colonianism}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
% ----------------------------------------------------
\imageframe{../img/settler_colonianism}
\note{}
% ----------------------------------------------------
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/fiat-concord-exportacion.jpg}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/asian_tigers}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
% ----------------------------------------------------
\begin{frame}
\frametitle{El modelo chino: ni ISI, ni Consenso de Washington}
\centering

\begin{itemize}
  \item China no sigui\'{o} ninguna receta est\'{a}ndar:
  \begin{itemize}
    \item Apertura comercial \textbf{gradual y selectiva} (no de shock)
    \item Estado activo: empresas p\'{u}blicas, pol\'{i}tica industrial, inversi\'{o}n masiva en infraestructura
    \item Control de capitales y tipo de cambio administrado
  \end{itemize}
  \vspace{8pt}
  \item<2-> \textbf{Resultados:} la mayor reducci\'{o}n de pobreza de la historia
  \begin{itemize}
    \item 800 millones de personas salieron de la pobreza extrema
    \item PIB multiplicado por 40 en cuatro d\'{e}cadas
  \end{itemize}
  \vspace{8pt}
  \item<3-> \textbf{Belt and Road Initiative} (2013): China como prestamista alternativo
  \begin{itemize}
    \item Infraestructura en \'{A}frica, Asia y Latinoam\'{e}rica
    \item Compite con el FMI y el Banco Mundial: \alert{sin condicionalidad pol\'{i}tica}
    \item Pero: cr\'{i}ticas por ``trampa de la deuda''
  \end{itemize}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{China es el caso mas importante del desarrollo contemporaneo y no encaja en las categorias clasicas. No fue ISI (China se abrio al comercio, pero de forma gradual y controlada, empezando por Zonas Economicas Especiales desde 1980). No fue el Consenso de Washington (el Estado mantuvo un papel central: empresas publicas en sectores estrategicos, politica industrial activa, control de capitales, tipo de cambio administrado). Fue un hibrido que algunos llaman ``capitalismo de Estado'' o ``Consenso de Pekin''. Los resultados son espectaculares: 800 millones de personas salieron de la pobreza extrema, el mayor exito de desarrollo de la historia. La Belt and Road Initiative (BRI, 2013) lleva este modelo al exterior: China financia infraestructura (puertos, carreteras, ferrocarriles) en paises en desarrollo, sin exigir reformas democraticas ni austeridad fiscal. Esto la convierte en una alternativa atractiva al FMI para muchos gobiernos. Pero hay criticas serias: los prestamos chinos pueden crear dependencia (la ``trampa de la deuda'': Sri Lanka cedio el puerto de Hambantota a China por 99 anos tras no poder pagar), y los proyectos a veces benefician mas a empresas chinas que a la economia local.}

% ----------------------------------------------------
\begin{frame}
\frametitle{La trampa de la renta media}
\centering

\begin{itemize}
  \item Muchos pa\'{i}ses han crecido hasta llegar a \textbf{renta media}\ldots{} y se han \textbf{estancado}
  \vspace{8pt}
  \item<2-> Ejemplos: \textbf{Brasil, M\'{e}xico, Turqu\'{i}a, Sud\'{a}frica, Argentina}
  \begin{itemize}
    \item Crecimiento r\'{a}pido con mano de obra barata e industrializaci\'{o}n b\'{a}sica
    \item Pero no logran el salto a econom\'{i}as de \textbf{alta productividad e innovaci\'{o}n}
  \end{itemize}
  \vspace{8pt}
  \item<3-> \textquestiondown Por qu\'{e}?
  \begin{itemize}
    \item La mano de obra ya no es tan barata (compiten con pa\'{i}ses m\'{a}s pobres)
    \item Instituciones insuficientes para innovar: educaci\'{o}n, I+D, Estado de derecho
    \item Desigualdad y captura de rentas por \'{e}lites
  \end{itemize}
  \vspace{8pt}
  \item<4-> Pocos han superado: \textbf{Corea del Sur, Taiwan, Singapur}
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{La trampa de la renta media es uno de los debates mas importantes del desarrollo contemporaneo. La idea: es relativamente facil crecer de pais pobre a pais de renta media --- basta con industrializarse, usar mano de obra barata, y abrirse al comercio. Pero el salto de renta media a renta alta es mucho mas dificil: requiere innovacion, capital humano cualificado, instituciones sofisticadas, y un sector servicios productivo. Brasil crecio rapidamente en los anos 60-70 con ISI, pero lleva decadas estancado. Mexico, a pesar del NAFTA y la apertura comercial, no ha convergido con EE.UU. Turquia crecio con fuerza en los 2000 pero se ha estancado. Los pocos paises que han escapado de la trampa (Corea del Sur, Taiwan, Singapur) invirtieron masivamente en educacion y tecnologia, mantuvieron instituciones relativamente inclusivas, y gestionaron la transicion de manufacturas basicas a sectores de alto valor anadido. La pregunta para China es si tambien caera en la trampa: su crecimiento se ha desacelerado, su poblacion envejece, y la transicion de ``fabrica del mundo'' a economia de innovacion esta en marcha pero no garantizada.}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/usaid}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
