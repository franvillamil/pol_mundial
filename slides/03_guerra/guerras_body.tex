...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/wwimap}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
\note{territorial loses by great power after wwi}

% ----------------------------------------------------
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/wwimappost}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
\note{...}
...
\begin{enumerate}
  \item Conflicto de intereses: por qué
  \item La guerra como un problema de la negociación
  \begin{itemize}
    \item Problemas de información (información incompleta y señales costosas)
    \item Problemas de compromiso (\textit{commitment problems})
    \item Bienes indivisibles
  \end{itemize}
  \item Coacción (\textit{compellence} \& \textit{deterrence})
  \item La guerra hoy en día
\end{enumerate}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/ukraineterritory}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
\note{Guerra de Ucrania}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{La guerra como problema de negociación}
\centering

\begin{itemize}
  \item Anarquía: No hay tribunales ni policía a nivel internacional
  \item Los Estados resuelven sus conflictos mediante \textbf{negociación bajo amenaza de fuerza} 
  \item<2-> Cómo se desarrolla esta negociación?
  \item<2-> \textit{Crisis bargaining} / diplomacia coercitiva
  \begin{itemize}
    \item Empieza cuando se entra en \textbf{crisis}, es decir, cuando un Estado amenaza con usar la fuerza
    \item Consiste generalmente en una escalada de amenazas $+$ ofertas de acuerdo
  \end{itemize}
\end{itemize}
...
% \end{itemize}
  ...
  \uncover<4->{\draw[->] (0, 1) -- (4.5, 1) node [above, midway] {\tiny Lo que espera ganar A};}
  \uncover<4->{\draw[<-] (7.5, 1) -- (10, 1) node [above, midway] {\tiny Lo que espera ganar B};}
  \uncover<5->{\draw[decorate,decoration={brace,amplitude=2pt}] (4.5, 2) -- (7.5, 2) node [above, midway, text width=2.3cm] {\tiny Espacio de negociación (cualquier acuerdo aquí es mejor que ir a la guerra)\\};}

  \end{tikzpicture}

  \begin{itemize}
\item<1> Imagina que A y B están luchando por el control de un territorio, y A es un poco más fuerte que B (y ambos lo saben)
\item<2> $p$ es lo que esperan obtener si pelean
\item<3-4> Pero la guerra tiene un costo, así que terminarían con un poco menos
\item<5> Por lo tanto, bajo condiciones racionales, estarían mejor si negocian antes de pelear
  \end{itemize}
...
\begin{itemize}
  ...
  \end{itemize}
  \item<4-> ¿Cuál es el ejemplo de política de disuasión más común? Relacionado con el modelo de negociación
\end{itemize}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Ejemplo de deterrence: armas nucleares}
\centering
...
\begin{itemize}
  ...
  \item<4-> \textbf{¿Cuál es la estrategia de disuasión de las potencias nucleares: UK/Francia, USA, Rusia, China?}
\end{itemize}
...
\begin{itemize}
  \item ¿Se os ocurre un conflicto real en el que ambos bandos habrían estado mejor con un acuerdo negociado?
  \item<2-> Ejemplo, guerra de Gaza: Israel-Hamas
  \item<2-> ¿Cuáles son los costos de la guerra para ambos bandos?
  \item<2-> ¿Qué van a ganar (o más bien, qué gana Israel)?
  \item<2-> ¿Hay un acuerdo posible al que podrían haber llegado?
  \item<3-> \textbf{¿Por qué fueron a la guerra entonces?}
\end{itemize}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/rebelchickenrun}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
\note{ejemplo}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/ulysses}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
\note{ejemplo}
...
\begin{itemize}
  ...
  \end{itemize}
  \vspace{8pt}
  \item<3-> Ejemplo: Kennedy aumentó el ejército en 350.000 soldados durante la crisis de Berlin (1961), Putin pre 2022
\end{itemize}
...
\begin{itemize}
  ...
  \vspace{8pt}
  \item<3-> Ejemplo (contrario): \textbf{Libia (2003-2011)}
  \begin{itemize}
...
\begin{itemize}
  ...
  \end{itemize}
  \vspace{8pt}
  \item<2-> El Estado que se debilita prefiere luchar \textbf{hoy} antes de ser más debil \textbf{mañana}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Guerra \textit{preemptive} (preemptiva?)}
\centering
...
Conflicto USA-China? Y qué mecanismos podrían reducir el riesgo de guerra?
...
\begin{itemize}
  ...
  \end{itemize}
  \item<3-> Esto es lo que vamos a ver la semana que viene
\end{itemize}
...
\begin{itemize}
  ...
  \item<3-> La guerra interestatal es menos frecuente, pero no ha desaparecido
\end{itemize}
