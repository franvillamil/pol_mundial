...
\end{frame}
% ----------------------------------------------------
\note{Introducción a la sesión. Conectar con la sesión anterior sobre normas y derecho internacional. Hoy pasamos de la seguridad a la economía política internacional. El comercio es el tema más antiguo de la economía política: ¿por qué intercambian los países? ¿Quién gana y quién pierde? ¿Por qué los gobiernos ponen trabas a algo que, según los economistas, genera ganancias netas? Cuatro bloques: el puzzle básico, ventaja comparativa, ganadores y perdedores, y la política del proteccionismo y la cooperación.}
...
\begin{itemize}
  ...
  \item El puzzle del comercio: ganancias netas, pero restricciones
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, width=\paperwidth]{../img/comerciointernacional}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, width=\paperwidth]{../img/balanza_comercial_esp}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
% IMAGE NEEDED: container_ship.jpg -- fotografía aérea de un gran buque portacontenedores en alta mar o en un puerto; preferiblemente con grúas de carga al fondo para subrayar la escala del comercio global
\note{importance of trade for an economy}




% ----------------------------------------------------
\begin{frame}<1-3>[label=puzzle]
...
\begin{itemize}
  ...
  \item<3-> Ejemplo: el caso de \textbf{la carne en Suiza}
  \begin{itemize}
    \item<4-> Producto agrícola, sector cárnico protegido por ser sector sensible
    \item<4-> Importaciones limitadas, aranceles muy altos
    \item<4-> Coste para los consumidores: kg carne de pollo en Suiza $\approx$ 25€, en Italia $\approx$ 8€
    \item<5-> Sobre 100.000 trabajadores en el sector? ($\approx$ 1\%)
    \item<5-> Sobrecoste consumo vs coste puestos de trabajos?
  \end{itemize}
  \item<6-> \textbf{Pregunta central:} ¿por qué hay limitaciones al comercio?
\end{itemize}
...
% ----------------------------------------------------
\imageframe{../img/switzerland-airport-customs}
\note{}
% ----------------------------------------------------

% ----------------------------------------------------
\imageframe{../img/swiss_drugs_meat}
\note{}
% ----------------------------------------------------

% ----------------------------------------------------
\imageframe{../img/coop_meat}
\note{}
% ----------------------------------------------------
  
% ----------------------------------------------------
\imageframe{../img/swiss_meat}
\note{}
% ----------------------------------------------------
...
\vspace{10pt}
\onslide<2->{\small El comercio crea \textbf{ganadores y perdedores}}
...
y se quedan con las ganancias netas? ¿Por qué ganan los perdedores?}
\end{frame}
% ----------------------------------------------------
...
\begin{itemize}
  \item David Ricardo (1817): \alert{\textbf{ventaja comparativa}}
  \begin{itemize}
  ...
  \item<4-> \textbf{Ojo}: no es lo mismo \textit{ventaja comparativa} que \textit{ventaja absoluta}
\end{itemize}
...
% ----------------------------------------------------
\imageframe{../img/ventaja_comparativa}
% ----------------------------------------------------
\note{}
...
\begin{itemize}
  \item ¿Pero cómo explicamos en \textbf{qué se especializa} cada país?
  \vspace{8pt}
  \item<2-> Modelo \textbf{Heckscher-Ohlin:} los países exportan bienes que usan intensivamente sus \textbf{factores abundantes}
  \item<2-> Tres factores de producción: \textbf{tierra}, \textbf{trabajo} y \textbf{capital}
  \vspace{8pt}
  \item<3-> Ejemplos:
  \begin{itemize}
  ...
  \end{itemize}
  \item<4-> Predice qué produce y exporta cada país según su \textbf{dotación de recursos}
\end{itemize}
...
\begin{itemize}
    \item Heckscher-Ohlin sólo predice la especialización de cada país en \textbf{industrias diferentes}, pero no todo el comercio es así
\end{itemize}
...
% % ----------------------------------------------------
% \begin{transitionframe}
% ¿Quién gana y quiénGanadores y perdedores del libre comercio
% \end{transitionframe}
% % ----------------------------------------------------
% \note{}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Ganadores y perdedores}
\centering
...
\begin{itemize}
  ...
  \item<5-> Ejemplo de la \textbf{carne en Suiza}? Y en España?
\end{itemize}
...
\textbf{Productores export.} & Acceso a mercados exteriores & Costes más altos (insumos) \\[5pt]
\textbf{Productores import.} & Competencia, posible cierre & Protección, precios altos \\[5pt]
\textbf{Trabajadores export.} & Más empleo, mejores salarios & Menos demanda exterior \\[5pt]
\textbf{Trabajadores import.} & Pérdida de empleo & Empleo protegido \\[5pt]
\end{tabular}
}
...
% ----------------------------------------------------
\begin{frame}<1-2>[label=proteccionismo]
\frametitle{Proteccionismo: cómo y por qué}
\centering

\begin{itemize}
    \item Es fácil ver que el proteccionismo tiene perdedores y ganadores
    \item ¿Son las barreras al comercio una protección a los perdedores?
    \item<2-> La carne en Suiza: poderosos granjeros suizos?
    \item<2-> No hay otras razones para ello?
    \vspace{10pt}
    \item<3-> Hay muchas razones para poner proteccionismo, y también formas de hacerlo más allá de aranceles
\end{itemize}
...
% ----------------------------------------------------
\imageframe{../img/taiwan_meat}
\note{}
% ----------------------------------------------------

% ----------------------------------------------------
\imageframe{../img/taiwan_meat2}
\note{}
% ----------------------------------------------------

% ----------------------------------------------------
\imageframe{../img/aduanas_chile}
\note{}
% ----------------------------------------------------

\againframe<3->{proteccionismo}
\note{}
...
\hline
Arancel & Impuesto sobre la importación & Acero en EE.UU. \\[4pt]
Cuota & Límite cuantitativo a las importaciones & Azúcar en la UE (carne en Suiza) \\[4pt]
...
\begin{itemize}
  \item En gran medida, por el \textbf{lobbying:}
  \begin{itemize}
    ...
    \item \alert{Problema de acción colectiva} de M. Olson
  \end{itemize}
  \vspace{-2pt}
  \item<2-> Pero hay más \textbf{argumentos ``válidos'':}
  \begin{itemize}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[at = (current page.center), yshift = 0cm] (cover) {%
      \includegraphics[keepaspectratio, height=\paperheight]{../img/trump_tariffs}};
  \end{tikzpicture}
\end{frame}
% ----------------------------------------------------
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Política doméstica}
\centering

\begin{itemize}[<+->]
    \item ¿Cómo afecta la \textbf{organización interna} de un país al \textbf{libre comercio?} Y sus \textbf{consecuencias?}
    \vspace{10pt}
    \item Incentivos electorales de líderes
    \item \textit{Cleavages} y posiciones sobre comercio
    \item Cómo se compensa el libre comercio
    \item Cómo se cuadra todo esto: \textbf{el trilema de Rodrik}
\end{itemize}
...
\begin{minipage}{.49\textwidth}\centering
  \includegraphics[height = .95\textheight]{../img/rodrik_shared}%\\
  % 2025\\
  % \vfill
\end{minipage}\hfill
% ----------------------------------------------------
\begin{minipage}{.49\textwidth}\centering
  \includegraphics[height = .95\textheight]{../img/rodrik_straight}%\\
  % 2018\\%{\tiny (Esp: \textit{Hablemos claro sobre el comercio mundial})}
  % \vfill
\end{minipage}
% ----------------------------------------------------
...
% ====================================================
\section{Organización internacional}
% ====================================================
...
\begin{tikzpicture}[>=stealth]
  % Grid: left column wider, right column normal
  \draw (1,0) -- (1,3) -- (8,3) -- (8,0) -- (1,0); % outer box
  \draw (1,1.5) -- (8,1.5);  % horizontal divider
  \draw (4.5,0) -- (4.5,3);  % vertical divider shifted right

  % Column headers (País B) — moved up/outside
  \node at (6.25, 3.6) {\textbf{País B}};
  \node at (2.75, 3.15) [font=\small, text=accent]  {Libre comercio};
  \node at (6.25, 3.15) [font=\small, text=accent2] {Proteccionismo};

  % Row headers (País A) — moved left/outside
  \node at (0.2, 1.5) {\textbf{País A}};
  \node at (0.6, 2.25) [font=\small, text=accent]  {Libre};
  \node at (0.6, 0.75) [font=\small, text=accent2] {Protec.};

  % Cell contents — centered in each cell
  \node at (2.75, 2.35) [font=\small]                   {+3, +3};
  \node at (2.75, 2.05) [font=\scriptsize, text=asher]  {óptimo};

  \node at (6.25, 2.25) [font=\small, text=accent2]     {-1, +4};

  \node at (2.75, 0.75) [font=\small, text=accent2]     {+4, -1};

  \node at (6.25, 0.85) [font=\small, text=accent2]     {0, 0};
  \node at (6.25, 0.55) [font=\scriptsize, text=accent2]{equilibrio};
\end{tikzpicture}
\vspace{10pt}
\begin{itemize}
    \item \textbf{Problema:} o nos coordinamos, o todos acabamos con barreras comerciales
    \item Cómo podemos conseguir cooperar?
\end{itemize}
...
\begin{tikzpicture}[>=stealth, thick]
  ...
  \node[below, font=\scriptsize, align=center] at (9, -0.2) {\textbf China\\ingresa};

  % Band showing tariff reductions
  \fill[accent!30, opacity=0.5] (0, 0.75) rectangle (10, 1.25);
  \node[font=\scriptsize, align=center] at (5, 1) {Aranceles de $\sim$40\% (1947) a $\sim$5\% (2000)};
\end{tikzpicture}
...
\small
\begin{itemize}
  \item \textbf{Nación más favorecida} [\textit{most-favored-nation (MFN) principle}]
  \begin{itemize}
...
\begin{itemize}
  ...
  \item La \textbf{ventaja comparativa} explica la especialización
  \item Los perdedores del comercio están \textbf{mejor organizados}
  \item El proteccionismo responde a lógicas políticas, no solo económicas
  \item Papel de la \textbf{OMC} en la cooperación internacional
